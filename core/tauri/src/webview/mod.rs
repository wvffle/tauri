...
use crate::{
  ...
  manager::{
    webview::{JsEventListenerKey, WebviewLabelDef},
    AppManager,
  },
...
```
  ...
  pub(crate) fn unlisten_js(&self, event: &str, id: EventId) -> crate::Result<()> {
    ...
    let mut empty = None;
    let mut js_listeners = self.manager().webview.js_event_listeners.lock().unwrap();
    if let Some(js_listeners) = js_listeners.get_mut(self.label()) {
  ...
  pub(crate) fn has_js_listener(&self, source: &EventSource, event: &str) -> bool {
    ...
    if let Some(js_listeners) = js_listeners.get_mut(self.label()) {
      match source {
        ...
        EventSource::Global => js_listeners.keys().any(|k| k.event == event),
        ...
        EventSource::Window { label } if label == self.window.label() => {
          ...
          js_listeners.contains_key(&JsEventListenerKey {
            source: source.clone(),
            event: event.clone(),
          }) || js_listeners.contains_key(&JsEventListenerKey {
