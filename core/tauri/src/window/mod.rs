...
use crate::manager::webview::JsEventListenerKey;
use crate::{
  app::AppHandle,
...
```
  ...
  /// Whether this window has any registered listener to an event from any webview.
  pub(crate) fn has_any_js_listener(&self, event: &str) -> bool {
    let js_event_listeners = self.manager.webview.js_event_listeners.lock().unwrap();
    js_event_listeners.values().any(|listeners| {
      listeners.contains_key(&JsEventListenerKey {
        event: event.to_string(),
        source: EventSource::Global,
      }) || self.webviews().iter().any(|w| {
        listeners.contains_key(&JsEventListenerKey {
          event: event.to_string(),
